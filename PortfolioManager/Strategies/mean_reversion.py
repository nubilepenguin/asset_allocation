...
def MeanReversionStrategy(contract, data_csv='',data_list=[], **strategy_params):
    """
    Run a mean reversion strategy on either the data_csv
    ...
    # dump out parameters, use defaults if missing
    if log_level > 0:
    ...
    for line in reversed(list(market_data)): #file is backwards
        try:
            date, open_price, high_price, low_price, close_price = fp.TokenizeToPriceInfo(contract,line)
